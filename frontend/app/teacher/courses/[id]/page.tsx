...
export default function CourseDetailPage() {
  ...
  const validateLessonForm = () => {
    ...
    if (lessonForm.video_type === 'upload' && !lessonForm.video_file && !editingLesson) {
      newErrors.video_file = 'يرجى اختيار ملف الفيديو';
    } else if (lessonForm.video_type !== 'upload' && !lessonForm.video_url.trim()) {
      newErrors.video_url = 'رابط الفيديو مطلوب';
    }

    if (!lessonForm.duration.trim()) {
      newErrors.duration = 'مدة الدرس مطلوبة';
    } else if (!/^([0-9]{1,2}:)?[0-5][0-9]:[0-5][0-9]$/.test(lessonForm.duration)) {
  ...
  const handleAddLesson = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/teacher/courses/${courseId}/lessons`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${authData.token}`,
  ...
  const handleUpdateLesson = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/teacher/lessons/${editingLesson.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${authData.token}`,
          ...
          body: JSON.stringify({
            ...
            video_url: lessonForm.video_type !== 'upload' ? lessonForm.video_url : undefined,
  ...
  const handleDeleteLesson = async (lessonId: number) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/api/teacher/lessons/${lessonId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${authData.token}`,
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 500 * 1024 * 1024) {
        setErrors(prev => ({ ...prev, video_file: 'حجم الملف يجب أن يكون أقل من 500 ميجابايت' }));
        return;
      }
  ...
  if (file) {
    if (file.size > 5 * 1024 * 1024) {
      setErrors(prev => ({ ...prev, thumbnail_file: 'حجم الملف يجب أن يكون أقل من 5 ميجابايت' }));
      return;
    }
          ...
          <div className={styles.courseHeader}>
            ...
            <div className={styles.courseDetails}>
              ...
              <div className={styles.courseMeta}>
                ...
                <span>•</span>
                <span className={course?.is_active ? styles.active : styles.inactive}>
                      ...
                      <div className={styles.lessonMeta}>
                        ...
                        <span>
                          ...
                          {lesson.video_type === 'youtube' ? 'YouTube' : 
                           lesson.video_type === 'vimeo' ? 'Vimeo' : 
                           lesson.video_type === 'upload' ? 'رفع مباشر' : 'رابط خارجي'}
                        </span>
                        {lesson.is_preview && (
              ...
              <form onSubmit={editingLesson ? handleUpdateLesson : handleAddLesson}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.videoTypeButtons}>
                    <button
                      type="button"
                      className={lessonForm.video_type === 'youtube' ? styles.active : ''}
                      onClick={() => setLessonForm(prev => ({ ...prev, video_type: 'youtube' }))}
                    >
                      <FaYoutube />
                      <span>YouTube</span>
                    </button>
                    <button
                      type="button"
                      className={lessonForm.video_type === 'vimeo' ? styles.active : ''}
                      onClick={() => setLessonForm(prev => ({ ...prev, video_type: 'vimeo' }))}
                    >
                      <span>Vimeo</span>
                    </button>
                    <button
                      type="button"
                      className={lessonForm.video_type === 'upload' ? styles.active : ''}
                      onClick={() => setLessonForm(prev => ({ ...prev, video_type: 'upload' }))}
                    >
                      <FaUpload />
                      <span>رفع فيديو</span>
                    </button>
                    <button
                      type="button"
                      className={lessonForm.video_type === 'embed' ? styles.active : ''}
                      onClick={() => setLessonForm(prev => ({ ...prev, video_type: 'embed' }))}
                    >
                      <FaFile />
                ...
                {lessonForm.video_type !== 'upload' ? (
                  <div className={styles.formGroup}>
                    ...
                    <input
                      ...
                      placeholder={
                        lessonForm.video_type === 'youtube' ? 'https://www.youtube.com/watch?v=...' :
                        lessonForm.video_type === 'vimeo' ? 'https://vimeo.com/...' :
                        'https://...'
                      }
                      className={errors.video_url ? styles.error : ''}
                    />
                    {errors.video_url && <span className={styles.errorMessage}>{errors.video_url}</span>}
                  </div>
                ) : (
                  <div className={styles.formGroup}>
                    ...
                    <div className={styles.uploadSection}>
                      ...
                      <label htmlFor="video-upload" className={styles.uploadButton}>
                        <FaUpload />
                        <span>{lessonForm.video_file ? lessonForm.video_file.name : 'اختر ملف الفيديو'}</span>
                      </label>
                      <small>الحد الأقصى: 500MB - MP4, MOV, AVI, WMV</small>
                    </div>
                    {errors.video_file && <span className={styles.errorMessage}>{errors.video_file}</span>}
                ...
                <div className={styles.formGroup}>
                  ...
                  <div className={styles.uploadSection}>
                    <input
                      ...
                      className={styles.fileInput}
                    />
                    <label htmlFor="lesson-thumbnail-upload" className={styles.uploadButton}>
                      <FaImage />
                      <span>{lessonForm.thumbnail_file ? lessonForm.thumbnail_file.name : 'اختر صورة للدرس'}</span>
                    </label>
                    <small>PNG, JPG حتى 5MB</small>
                  </div>
                  {errors.thumbnail_file && <span className={styles.errorMessage}>{errors.thumbnail_file}</span>}
                </div>
                  ...
                  >
                    {isSubmitting ? (
                      <>
                        ...
                        <span>{editingLesson ? 'جاري التحديث...' : 'جاري الإضافة...'}</span>
                      </>
                    ) : (
                      <span>{editingLesson ? 'حفظ التغييرات' : 'إضافة الدرس'}</span>
                    )}
                  </button>
                  <button 
                    type="button" 
